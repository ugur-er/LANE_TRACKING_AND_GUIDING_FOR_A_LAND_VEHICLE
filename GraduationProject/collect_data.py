...
data_folder_train = os.path.join("data","train")
data_folder_lbl = os.path.join("data","train_label")

def main():
    mkdir_if_not_exist(data_folder)
    mkdir_if_not_exist(data_folder_train)
    mkdir_if_not_exist(data_folder_lbl)
    actor_list = []
    pygame.init()
    ...
    weather_cycle=0 #every 200 cycle weather will change
    weather_number=0
    try:
        m = world.get_map()
        # plot_map(m)
        ...
        with CarlaSyncMode(world, camera_rgb, fps=30) as sync_mode:
            ...
            while True:
                if should_quit():
                    return
                clock.tick()
                # set weather to synchronously
                if(weather_cycle==100):
                    weather_cycle=0
                    weather_number=(weather_number+1)%10
                    weather_preset, weather_preset_str = find_weather_presets()[weather_number]
                    weather_preset_str = weather_preset_str.replace(" ", "_")
                    world.set_weather(weather_preset)
                    simulation_identifier = (
                        town_string + "_" + weather_preset_str + "_" + date_time_string
                ...
                if store_files:
                    ...
                    image_out_path = os.path.join(
                        data_folder_train, filename_base + ".png"
                    )
                    save_img(image_rgb, image_out_path)
                    # label img
                    label_path = os.path.join(
                        data_folder_lbl, filename_base + "_label.png"
                    )
                    save_label_img(
